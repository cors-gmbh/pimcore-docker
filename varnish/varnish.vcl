...
backend server1 { # Define one backend
  .host = "{{ include "pimcore.fullname" . }}-php";   # IP or Hostname of backend
...
sub vcl_recv {
  ...
  if (req.http.Cookie) {
      ...
      #set req.http.Cookie = regsuball(req.http.Cookie, ";(PHPSESSID|_pc_tss)=", "; \1=");
      set req.http.Cookie = regsuball(req.http.Cookie, ";[^ ][^;]*", "");
      set req.http.Cookie = regsuball(req.http.Cookie, "^[; ]+|[; ]+$", "");
  ...
  if (req.url ~ "^[^?]*\.(7z|avi|avif|bz2|flac|flv|gz|mka|mkv|mov|mp3|mp4|mpeg|mpg|ogg|ogm|opus|rar|tar|tgz|tbz|txz|wav|webm|xz|zip)(\?.*)?$") {
    unset req.http.Cookie;
    return (hash);
  }
  ...
  if (req.url ~ "^[^?]*\.(7z|avi|avif|bmp|bz2|css|csv|doc|docx|eot|flac|flv|gif|gz|ico|jpeg|jpg|js|less|mka|mkv|mov|mp3|mp4|mpeg|mpg|odt|otf|ogg|ogm|opus|pdf|png|ppt|pptx|rar|rtf|svg|svgz|swf|tar|tbz|tgz|ttf|txt|txz|wav|webm|webp|woff|woff2|xls|xlsx|xml|xz|zip)(\?.*)?$") {
    unset req.http.Cookie;
    return (hash);
  }
...
sub vcl_deliver {
  ...
  unset resp.http.Age;
  unset resp.http.Server;
  unset resp.http.x-pimcore-output-cache-tag;
  unset resp.http.x-pimcore-cache-date;
  unset resp.http.x-debug-token;
  unset resp.http.x-cache-hits;

  set resp.http.Server = "CORS.gmbh";
  set resp.http.X-Powered-By = "CORS.gmbh";
